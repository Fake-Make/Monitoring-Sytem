...
3. Переходим в директорию бд в папку bin и запускаем сервер бд mongod.exe. Сервер запустится на localhost-e на стандартном порте.
4. Запускаем MongoDB Compass и нажимаем Connect.
5. Создаем бд с названием systDB и коллекцию readings (названия можно любые но тогда нужно будет их заменить в коде)
